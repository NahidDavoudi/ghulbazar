/**
 * ╔═══════════════════════════════════════════════════════════╗
 *   Ghul Bazar — app.js (frontend) - سازگار با متغیرهای CSS
 *   تمام رنگ‌های هاردکد شده با کلاس‌های مبتنی بر متغیر جایگزین شدند.
 * ╚═══════════════════════════════════════════════════════════╝
 */
...
;(function () {
  ...
  function _buildHeader() {
    ...
    const navItems = NAV_LINKS.map(l => `
      <a href="${l.href}" data-link
         class="text-sm text-dim hover:text-body transition-colors
                px-1 py-0.5 rounded hover:bg-black/5 header-nav-link">
        ${l.label}
      </a>`).join('');
    ...
    const userArea = loggedIn
      ...
      : `<a href="login.html"
            class="text-xs px-4 py-2 border border-border rounded-lg text-muted
                   hover:border-accent hover:text-body transition-all">
           ورود
         </a>`;
      ...
      <header class="sticky top-0 z-50 bg-body/90 backdrop-blur-md border-b border-border">
        <div class="max-w-[1280px] mx-auto px-4 md:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <a href="#/" data-link class="flex items-center gap-2 shrink-0">
            ...
            <span class="font-display text-lg text-body tracking-wider hidden sm:inline">غول بازار</span>
          </a>
          ...
          <div class="flex items-center gap-3">
            ...
            <a href="#/cart" data-link
              ...
              <svg width="20" height="20" viewBox="0 0 16 20" fill="none"
                   class="text-muted group-hover:text-body transition-colors">
              ...
              <span id="cart-badge"
                    class="hidden absolute -top-1 -right-1 w-4 h-4 bg-accent text-white
            ...
            <button id="mobile-menu-btn"
                    class="md:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-black/5 transition-colors">
        ...
        <!-- منوی موبایل -->
        <div id="mobile-menu"
             class="hidden md:hidden bg-surface border-t border-border px-4 py-3">
          <nav class="flex flex-col gap-1">
            ${NAV_LINKS.map(l => `
              <a href="${l.href}" data-link
                 class="py-2 px-3 rounded-lg text-sm text-dim
                        hover:bg-black/5 hover:text-body transition-colors">
                ${l.label}
              </a>`).join('')}
          </nav>
        </div>
  ...
  window.injectHeader = function () {
    ...
    function _highlightNav() {
      ...
      container.querySelectorAll('.header-nav-link').forEach(a => {
        ...
        a.classList.toggle('text-body', match);
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
            ...
            <div class="text-right">
              <div class="flex items-center gap-2 justify-end mb-4">
                <span class="font-display text-lg text-body">غول بازار</span>
            ...
            <div class="text-right">
              ...
              <p class="text-sm text-muted mb-2">پشتیبانی ۷ روز هفته</p>
              <p class="text-sm text-body" dir="ltr">@ghulbazar</p>
            </div>
          </div>

          <div class="border-t border-border pt-6 flex flex-col sm:flex-row
                      items-center justify-between gap-3 text-xs text-muted/60">
  ...
  window.productCard = function (p) {
    ...
    const badge   = p.badge
      ? `<span class="absolute top-3 right-3 bg-accent text-white text-[10px] font-bold px-2 py-0.5 rounded-full z-10">
           ${p.badge}
         </span>`
      : '';
    const lowStock = p.stock <= 2 && p.stock > 0
      ? `<span class="absolute top-3 left-3 bg-black/60 text-accent text-[10px] font-bold px-2 py-0.5 rounded-full z-10">
           آخرین موجودی
         </span>`
      : '';
    const outOfStock = p.stock === 0
      ? `<div class="absolute inset-0 bg-black/60 flex items-center justify-center rounded-xl z-10">
           <span class="text-sm text-white/70 font-medium">ناموجود</span>
         </div>`
      : '';

    return `
      <a href="${href}" data-link
         class="group block bg-card border border-border rounded-xl overflow-hidden
                hover:border-accent/40 transition-all duration-300
                hover:shadow-[0_8px_30px_rgba(75,107,138,0.12)]">
        ...
        <div class="p-3 md:p-4 text-right">
          ...
          <div class="flex items-center justify-between">
            <button
              class="add-to-cart-quick w-7 h-7 rounded-lg bg-transparent border border-border
  ...
  /* ══════════════════════════════════════════════════════════
     ستاره‌ها (اکنون از رنگ accent استفاده می‌کند)
  ══════════════════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════════════════
     بازسازی هدر بعد از هر navigation (تنهای active link)
  ══════════════════════════════════════════════════════════ */
